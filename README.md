...
The mechanism is the standard's `ctx_token`: an opaque click-token the agent issues, carried to the landing page in place of the session identifier, which a telemetry consumer resolves to the click manifest (the session's grounded, cited, and displayed sources). The session UUID never crosses the boundary.
